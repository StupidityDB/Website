...
const ReportsPage = (): JSX.Element => {
  ...
  return <main className='flex-1 max-w-5xl w-full mx-auto p-4 sm:p-6'>
    ...
    {selected.length > 0 && <div className='card px-4 py-3 mb-3 flex flex-wrap items-center gap-2'><span className='text-sm text-slate-300 mr-auto'>{selected.length} selected</span><button onClick={() => { void dismiss(selected) }} className='px-3 py-1.5 rounded text-sm bg-slate-700 hover:bg-slate-600 text-white'>Dismiss</button><button onClick={() => { void applyBulkAction('delete') }} className='px-3 py-1.5 rounded text-sm bg-red-700 hover:bg-red-600 text-white'>Delete reviews</button><select value={bulkBanDays} onChange={event => setBulkBanDays(Number(event.target.value) as 1 | 3 | 7 | 30)} className='bg-surface-2 border border-white/10 rounded px-2 py-1.5 text-sm text-slate-200' aria-label='Bulk ban duration'><option value={1}>1 day</option><option value={3}>3 days</option><option value={7}>1 week</option><option value={30}>1 month</option></select><button onClick={() => { void applyBulkAction('deleteAndBan') }} className='px-3 py-1.5 rounded text-sm bg-red-600 hover:bg-red-500 text-white'>Delete + ban authors</button></div>}
    <section className='card overflow-hidden'>
      {loading ? <p className='p-5 text-slate-400'>Loading reports…</p> : reports.length === 0 ? <p className='p-5 text-slate-400'>No reports on this page.</p> : reports.map(report => <article key={report.id} className='p-4 border-b border-white/5 last:border-0 flex gap-3'>
        <input type='checkbox' checked={selected.includes(report.id)} onChange={() => toggleSelected(report.id)} aria-label={`Select report ${report.id}`} className='mt-3 h-4 w-4 accent-blurple' />
        ...
        <button onClick={() => { void dismiss([report.id]) }} className='self-start px-2 py-1.5 rounded text-xs text-slate-400 hover:bg-white/10 hover:text-slate-200'>Dismiss</button>
      </article>)}
    </section>
