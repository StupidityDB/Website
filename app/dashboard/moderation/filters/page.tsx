...
const FiltersPage = (): JSX.Element => {
  ...
  return <main className='flex-1 max-w-5xl w-full mx-auto p-4 sm:p-6'><Link href='/dashboard/moderation' className='link text-sm'>Moderation home</Link><h1 className='text-2xl gg-bold text-white mt-4 mb-5'>Word filters</h1><div className='grid lg:grid-cols-3 gap-4'>{groups.map(group => <section key={group.type} className='card p-4'><h2 className='gg-semibold text-slate-100'>{group.title}</h2><div className='flex flex-col sm:flex-row gap-2 my-3'><input value={words[group.type]} onChange={event => setWords({ ...words, [group.type]: event.target.value })} onKeyDown={event => event.key === 'Enter' && add(group.type)} placeholder='Add word' className='min-w-0 flex-1 bg-surface-3 border border-white/10 rounded px-2 py-2 text-sm text-slate-100' /><button onClick={() => { void add(group.type) }} className='button !py-2 text-sm justify-center'>Add</button></div><div className='flex flex-wrap gap-2'>{filters?.[group.key]?.map(word => <button key={word} onClick={() => { void remove(word, group.type) }} title='Remove word' className='max-w-full break-all px-2 py-1 text-xs rounded bg-white/5 text-slate-300 hover:bg-red-500/20 hover:text-red-300'>{word} ×</button>)}</div></section>)}</div><ToastContainer /></main>
}
