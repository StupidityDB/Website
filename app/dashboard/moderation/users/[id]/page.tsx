...
const UserModerationPage = (): JSX.Element => {
  ...
  return <main className='flex-1 max-w-5xl w-full mx-auto p-4 sm:p-6'>
    ...
    {!user ? <p className='mt-6 text-slate-400'>Loading user or user not found.</p> : <>
      <section className='card p-4 my-5 flex flex-col sm:flex-row sm:items-center gap-4'><Image src={user.profile_photo || '/defaultAvatar.png'} alt='' width={52} height={52} className='rounded-full' unoptimized /><div className='flex-1 min-w-0'><h1 className='text-xl gg-bold text-white break-words'>{user.username}</h1><p className='text-sm text-slate-400 break-all'>{user.discord_id} · {reviewCount} reviews written · reputation {user.reputation}</p></div>{user.type === -1 ? <button onClick={() => { void setBanState(false) }} className='button w-full sm:w-auto justify-center !bg-emerald-600'><FaCheck />Unban</button> : <button onClick={() => { void setBanState(true) }} className='button w-full sm:w-auto justify-center !bg-red-600'><FaBan />Ban</button>}</section>
      <section className='card overflow-hidden'><div className='px-4 py-3 border-b border-white/10 text-sm gg-semibold text-slate-200'>Reviews written by this user</div>{reviews.length === 0 ? <p className='p-4 text-sm text-slate-400'>No reviews written by this user.</p> : reviews.map(review => <article key={review.id} className='p-4 border-b border-white/5 last:border-0 flex gap-3'><div className='flex-1 min-w-0'><p className='text-slate-200 whitespace-pre-wrap break-words'>{review.comment}</p><p className='mt-2 text-xs text-slate-400'>#{review.id} · {new Date(review.timestamp * 1000).toLocaleString()}</p></div><button onClick={() => { void removeReview(review) }} className='p-2 text-red-400 hover:bg-red-500/10 rounded self-start flex-shrink-0'><FaTrash /></button></article>)}</section>
    </>}
    <ToastContainer />
  </main>
}
