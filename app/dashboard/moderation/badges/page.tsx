...
const BadgesPage = (): JSX.Element => {
  ...
  return <main className='flex-1 max-w-5xl w-full mx-auto p-4 sm:p-6'><Link href='/dashboard/moderation' className='link text-sm'>Moderation home</Link><h1 className='text-2xl gg-bold text-white mt-4'>Badge management</h1><form onSubmit={add} className='card p-4 grid sm:grid-cols-2 gap-3 my-5'>{Object.entries(badge).filter(([key]) => key !== 'type').map(([key, value]) => <input key={key} required={key !== 'redirectURL'} value={value} onChange={event => setBadge({ ...badge, [key]: event.target.value })} placeholder={key} className='min-w-0 bg-surface-3 border border-white/10 rounded px-3 py-2 text-slate-100' />)}<input type='number' value={badge.type} onChange={event => setBadge({ ...badge, type: Number(event.target.value) })} placeholder='type' className='min-w-0 bg-surface-3 border border-white/10 rounded px-3 py-2 text-slate-100' /><button className='button w-full sm:w-auto justify-center'>Add badge</button></form><section className='card overflow-hidden'>{badges.map(item => <div key={item.id} className='p-3 border-b border-white/5 flex flex-wrap sm:flex-nowrap items-center gap-3'><img src={item.icon} alt='' className='w-7 h-7 object-contain flex-shrink-0' /><div className='flex-1 min-w-0'><p className='text-slate-100 break-words'>{item.name}</p><p className='text-xs text-slate-400 break-all'>{item.targetDiscordID} · {item.description}</p></div><button onClick={() => { void remove(item.id) }} className='w-full sm:w-auto text-left sm:text-right text-red-400 text-sm'>Delete</button></div>)}</section><ToastContainer /></main>
}
